...
pub fn run_next_app() {
    ...
    unsafe { load_app(app_manager.current_app) };
    let current_app = get_current_app();
    info!("[kernel] Running app_{}", current_app);
    let current_app = current_app as *const ();
    let current_app = current_app as usize;
    let current_app = current_app as extern "C" fn() -> !;
    current_app();
}